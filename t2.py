...
def get_checkout_info(bearer_token, uid, proxy=None):
    ...
    payload = {
        "data": {"web": True, "type": "full", "bucksBreakdown": {"bucksBack": False, "earned": False, "promo": False},
                 "buyerAddress": None, 
                 "bookIds": ["a4209e9e-95e3-4deb-8507-7681958b8f3e-7JGqaFCZkmfcdAbjKfoYc2bCdmG3"],
...
def confirm_stripe_payment(payment_intent_id, client_secret, card_number, card_exp_month, card_exp_year, card_cvc, proxy=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Card is live but needs 3DS — log separately as CCN, don't drop
            if status == 'requires_action':
                return False, response_data, "CCN - Card Live (3DS Required)"
            
            # Only true success is 'succeeded'
            if status == 'succeeded':
                return True, response_data, "Payment Successful"
            else:
                # Any other status on HTTP 200 is ambiguous — don't save as approved
                return False, response_data, f"Ambiguous Status: {status}"
        ...
        # Only save cards where the decline proves the card details are valid
        # processing_error removed — it's a server error, not card validation
        ...
        if decline_code in saveable_decline_codes:
            reason = f"CCN - {decline_code.replace('_', ' ').title()}"
            return True, response_data, reason
        else:
            return False, response_data, f"Declined: {decline_code or 'unknown'}"
